...
DOCKER_IMAGE=$(echo $BUILD_TAG | tr '[:upper:]' '[:lower:]'}).cpu.ppc64le
DOCKER_FILE="Dockerfile.cpu.ppc64le"
DOCKER_BINARY="docker"
if [ "${TF_NEED_CUDA}" == "1" ]; then
  DOCKER_IMAGE=$(echo $BUILD_TAG | tr '[:upper:]' '[:lower:]'}).gpu.ppc64le
  DOCKER_BINARY="nvidia-docker"
  DOCKER_FILE="Dockerfile.gpu.ppc64le"
fi
...
${DOCKER_BINARY} run \
  ...
  -e "TF_BUILD_BAZEL_TARGET=${TF_BUILD_BAZEL_TARGET}" \
